...
//  		redirect: 'follow'
	...
	log.trace("save_update", requestOptions)

fetch(url, requestOptions)
  .then(response => response.text())
  .then(result => {
  	log.trace('SAVE_UPDATE', result)
    if (result) {
  	   // temp: NEED A BETTER WAY TO rout back to after save
      router.push('/search?size=n_20_n')
    }
  })
  .catch(error => log.trace('error', error));
...
export function find(uuid) {
  ...
  //log.trace(req)
      ...
      log.trace('QUERY',JSON.stringify(queryBody))
      var myHeaders = new Headers();
            myHeaders.append("Authorization", "Bearer " + getAuth());
            myHeaders.append("Content-Type", "application/json");
      var requestOptions = {
