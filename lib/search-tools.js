        ...
        if (field[1].type === "value") {   // normal value aggs
            ...
            if (field[1]["ranges"]) {
                ...
                if (field[1].hasOwnProperty('uiRange')) {
                    const histogramInterval = field[1]['uiInterval'] ?? 1
                    requestBody.agg(esb.histogramAggregation(field[0] + "_histogram", field[0], histogramInterval).extendedBounds(field[1]['uiRange'][0], field[1]['uiRange'][1]));
                }
