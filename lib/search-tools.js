...
export function esQueryBuilder(request, config, authState) {
    ...
    if (request.filters && request.filters.length > 0) {
        ...
        for (const filter of request.filters) {
            const facetConfig = findFacet(filter.field, facetConfigs)
            if (!facetConfig) {
                continue
            }
    ...
    if (config.includeFilters && config.includeFilters.length > 0) {
        ...
        for (let i = 0; i < includeFilters.length; i++) {
            ...
            f[i] = esb.termsQuery(
                includeFilters[i].field,
                includeFilters[i].values
            )
        }
    ...
    if (config.excludeFilters && config.excludeFilters.length > 0) {
        ...
        for (let i = 0; i < excludeFilters.length; i++) {
            switch (excludeFilters[i].type) {
                case 'term':
                    f[i] = esb.termsQuery(
                        excludeFilters[i].field,
                        excludeFilters[i].values
                    )
                    break
                case 'exists':
...
function buildFacetAggregations(facets, request, authState, postFilters) {
    const aggs = []

    for (const [name, facetConfig] of Object.entries(facets)) {
        ...
        if (facetConfig.facetType !== 'group') {
            ...
            if (Array.isArray(facetConfig.isAggregationActive)) {
                // Array of functions
                isAggregationActive = facetConfig.isAggregationActive.some(
                    (f) => f(request.filters, authState)
                )
        ...
        switch (facetConfig.facetType) {
            ...
            case 'hierarchy':
                ...
                if (!facetConfig.groupByField) {
                    throw new Error(
                        'groupByField is required for hierarchy facets'
                    )
                }

                // Primary aggregation
                const hierAgg = esb.termsAggregation(
                    name,
                    facetConfig.groupByField
                )
            ...
            case 'megahierarchy':
                if (
                    !facetConfig.hierarchyFields ||
                    facetConfig.hierarchyFields.length < 2
                ) {
                    throw new Error(
                        'hierarchyFields must have at least 2 entries for megahierarchy facets'
                    )
                }

                const megaAgg = buildMegaHierarchyAgg(
                    name,
                    facetConfig.hierarchyFields,
                    facetConfig.aggregationSize
                )
            ...
            case 'group':
                // recursively build aggregations for child facets
                const groupAggs = buildFacetAggregations(
                    facetConfig.facets,
                    request,
                    authState,
                    postFilters
    ...
    switch (facetConfig.facetType) {
        ...
        case 'megahierarchy':
            return esb.termsQuery(
                facetConfig.hierarchyFields
                    ? facetConfig.hierarchyFields.at(-1) // leaf field
                    : facetConfig.field,
                filter.values
            )
...
/**
 * Finds the facet configuration for a given field name.
 *
 * @param {string} field - The field name to find the facet configuration for.
 * @param {Object} facets - The facets configuration object.
 * @returns {Object|undefined} - The facet configuration object if found, otherwise undefined.
 */
export function findFacet(field, facets) {
    ...
    const groupedFacets = Object.values(facets).filter(
        (f) => f.facetType === 'group'
    )
    ...
    // field may be one of the hierarchyFields of a megahierarchy facet
    for (const f of Object.values(facets)) {
        if (
            f.facetType === 'megahierarchy' &&
            f.hierarchyFields?.includes(field)
        ) {
            return f
        }
    }
...
/**
 * Recursively builds nested Elasticsearch term aggregations from an ordered array of fields.
 *
 * The root aggregation is named after the facet config key (e.g. `dataset_type`), while
 * each subsequent level is named after its own field string. The last field in the array
 * is the leaf and produces no further sub-aggregations.
 *
 * @param {string} name - The name for this aggregation node. Should be the facet config key
 *   for the root call, and the field string for all recursive calls.
 * @param {string[]} fields - Ordered list of Elasticsearch field names from root to leaf
 *   (e.g. `['dataset_type_hierarchy.first_level.keyword', 'dataset_type_hierarchy.second_level.keyword']`).
 *   Must contain at least one entry.
 * @param {number} [size=DEFAULT_TERM_SIZE] - Maximum number of buckets to return at each level.
 * @returns {esb.TermsAggregation} The root terms aggregation with all sub-aggregations nested inside.
 *
 */
function buildMegaHierarchyAgg(name, fields, size = DEFAULT_TERM_SIZE) {
    const [head, ...tail] = fields
    const agg = esb.termsAggregation(name, head).size(size)
    if (tail.length > 0) {
        agg.agg(buildMegaHierarchyAgg(tail[0], tail, size))
    }
    return agg
}

// just performs a simple ES search with a field
export function simple_query_builder(
    field,
    query,
    includeProperties = [],
    excludeProperties = []
    ...
    requestBody.query(esb.boolQuery().must(esb.matchQuery(field, query)))
...
export class Sui {
    ...
    static expandDefault(facet) {
        return facet[1].hasOwnProperty('isExpanded')
            ? facet[1]['isExpanded']
            : true
    }
    ...
    static applyFilters(
        ...
        if (filters && filters.length > 0) {
            ...
            for (let f in suiFilters) {
                if (
                    suiFilters[f].from !== undefined ||
                    suiFilters[f].to !== undefined
                ) {
                    // Range filter
