...
//  console.log('searchfields:', queryConfig.search_fields)
  let CURRENT = from //request.current
  let SIZE = 100
    ...
    values.forEach((v) => {
      ...
      if (facet_info[1]["type"] === 'range') {
        var element = createRangeQuery(v, facet_info)
          ...
          //console.log("DEFINE RANGEQ", element)
...
function createRangeQuery(value, facet_info) {
   console.log('createRangeQuery', value, facet_info)

   var RANGES = facet_info[1]["ranges"]

   console.log('RANGES', RANGES, value)
    var found= RANGES.find(function(element, index) {

      console.log('LOOKING..', element);
...
function checkFilterType(field, facets) {
    ...
    var found= facets.find(function(element, index) {
      ...
      //console.log(element[0], field);
      if(element[0]=== field)
        //return element[1]["type"];
      return element;
    });

    //console.log('YEP ITS HERE', found)
    if (found) {
      //return found[1]["type"]
      return found
    }
