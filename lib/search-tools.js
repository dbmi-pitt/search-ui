    ...
    if (config.facets) {
        ...
        for (const [name, facetConfig] of Object.entries(config.facets)) {
            if (facetConfig.facetType === 'group') {
                for (const [subName, subFacetConfig] of Object.entries(
                    facetConfig.facets
                )) {
                    flattenedFacets[subName] = subFacetConfig
                }
            } else {
                flattenedFacets[name] = facetConfig
            }
        }

        for (const [name, facetConfig] of Object.entries(flattenedFacets)) {
            ...
            switch (facetConfig.facetType) {
                case 'term':
                    ...
                    termAgg.size(
                        facetConfig.aggregationSize ?? DEFAULT_TERM_SIZE
                    )
                ...
                case 'hierarchy':
                    ...
                    // Sub-aggregation
                    const subAgg = esb.termsAggregation(
                        'subagg',
                        facetConfig.field
                    )
                    subAgg.size(
