        ...
        values.forEach((v) => {
            ...
            if (facet_info[1] && facet_info[1]["type"] === 'range') {
                ...
                if (values.length > 1) {
                    ...
                    requestBody.postFilter(esb.termQuery(field, v))
                }
            }
        })
        if (values.length > 1) {
            requestBody.postFilter(filterBoolQuery)
        }
    });
    ...
    FACETS.map((field) => {
        if (field[1].type === "value") {   // normal value aggs
            ...
            if (field[1]["ranges"]) {
                ...
                if (field[1].hasOwnProperty('uiRange')) {
                    requestBody.agg(esb.histogramAggregation(field[0] + "_histogram", field[0], 1).extendedBounds(field[1]['uiRange'][0], field[1]['uiRange'][1]));
                }
...
export class Sui {
    ...
    static isExpandedDateCategory(facet, category) {
        ...
        return f.isExpanded === true || f.hasOwnProperty("startdate") || f.hasOwnProperty("enddate") || Sui.expandDefault(facet)
    }

    static isExpandedNumericCategory(facet, category) {
        const filters = Sui.getFilters()
        const f = filters[category] ?? {}
        return f.isExpanded === true || f.hasOwnProperty("min") || f.hasOwnProperty("max") || Sui.expandDefault(facet)
    }
