    ...
    if (config.facets) {
        ...
        for (const [name, facetConfig] of Object.entries(config.facets)) {
            ...
            if (facetConfig.aggregation) {
                ...
                let isActive = true
                if (Array.isArray(facetConfig.aggregation.isActive)) {
                    // Array of functions
                    isActive = facetConfig.aggregation.isActive.some((f) => f(request.filters, authState))
                } else if (typeof facetConfig.aggregation.isActive === 'function') {
                    // Function
                    isActive = facetConfig.aggregation.isActive(request.filters, authState)
                } else {
                    // Boolean
                    isActive = facetConfig.aggregation.isActive
                }
