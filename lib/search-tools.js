...
//  log.trace('searchfields:', queryConfig.search_fields)
    ...
    values.forEach((v) => {
      ...
      if (facet_info[1]["type"] === 'range') {
        var element = createRangeQuery(v, facet_info)
          ...
          //log.trace("DEFINE RANGEQ", element)
...
function createRangeQuery(value, facet_info) {
  log.trace('createRangeQuery', value, facet_info)

   var RANGES = facet_info[1]["ranges"]

  log.trace('RANGES', RANGES, value)
    var found= RANGES.find(function(element, index) {

      log.trace('LOOKING..', element);
      if (element["name"] === value) {
        log.trace("VALUES", element)
        return element
      }
    });
    return found
...
function checkFilterType(field, facets) {
    ...
    var found= facets.find(function(element, index) {
      ...
      //log.trace(element[0], field);
      if(element[0]=== field)
        //return element[1]["type"];
      return element;
    });

    //log.trace('YEP ITS HERE', found)
    if (found) {
      //return found[1]["type"]
      return found
    }
