...
export function es_query_builder(request, from, size, queryConfig) {
    ...
    if (GROUP_BY) {
        requestBody.agg(esb.termsAggregation(GROUP_BY, GROUP_BY).aggregations(aggregation))
    } else {
    ...
    let fullQuery = requestBody.toJSON()
    if (RAW) {
        fullQuery[RAW.key][RAW.subKey] = RAW.query
    }

    //log.debug('es_query_builder', fullQuery);
    return fullQuery;
}
