    ...
    FACETS.map((field) => {
        if (field[1].type === "value") {   // normal value aggs
            ...
            if (DISJUNCTIVE_FACETS && DISJUNCTIVE_FACETS.includes(field[0])) {
                requestBody.agg(esb.termsAggregation(field[0], field[1].field).minDocCount(0))
            } else {
...
export class Sui  {
    ...
    static isExpandedFacetCategory(facet, category) {
        ...
        return isExpanded || (facet[1].hasOwnProperty("isExpanded") ? facet[1]["isExpanded"] : true)
    }
    ...
    static isExpandedDateCategory(facet, category) {
        ...
        return (isExpanded > 0) || (facet[1].hasOwnProperty("isExpanded") ? facet[1]["isExpanded"] : true)
    }

    static applyFilters(setFilter) {
        const filters = Sui.getFilters()
        for (let f in filters) {
            if (filters[f].selected) {
                setFilter(filters[f].key, f)
            }
        }
    }
}
